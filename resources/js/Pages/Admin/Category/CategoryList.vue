...
const deleteCategory = (item) => {
    ElMessageBox.confirm(
        'Tindakan ini tidak dapat dibatalkan. Yakin ingin menghapus kategori ini?',
        'Hapus Kategori',
        {
    ...
    <div class="p-4 lg:p-6 bg-surface">
        ...
        <div class="mb-6">
            <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                <div>
                    <h1 class="text-xl lg:text-2xl font-bold text-on-surface">Kategori</h1>
                    <p class="text-xs text-on-surface-variant mt-1">Kelola kategori produk untuk toko Anda</p>
                </div>
        ...
        <div class="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
            <div class="bg-white border border-outline-variant rounded p-4">
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        <p class="text-[10px] font-bold text-on-surface-variant uppercase tracking-wider">Total Kategori</p>
                        <p class="text-base font-bold text-on-surface">{{ categories.total }}</p>
                    </div>
                </div>
            </div>
        </div>
            ...
            <div class="overflow-x-auto">
                <table class="w-full">
                    ...
                    <tbody class="divide-y divide-outline-variant">
                        <tr v-for="(item, index) in categories.data" :key="item.id" class="hover:bg-surface transition-colors">
        ...
        <el-dialog v-model="dialogVisible" width="500px" class="!rounded">
            <template #header>
                <h3 class="text-sm font-bold text-on-surface uppercase tracking-wider">
                    {{ isEditItem ? 'Edit Kategori' : 'Tambah Kategori' }}
                </h3>
            </template>

            <form @submit.prevent="isEditItem ? updateCategory() : AddCategory()" class="space-y-5">
                <div>
                    <label class="block text-[10px] font-bold text-on-surface-variant uppercase tracking-wider mb-2">Nama Kategori</label>
